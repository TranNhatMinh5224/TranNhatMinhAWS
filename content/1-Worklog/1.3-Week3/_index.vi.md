...
| :--- | :--- | :--- |
| **Thứ 2 (17/08)** | • Nghiên cứu lý thuyết phân chia mạng con CIDR và Subnet Masking (/16, /24, /28).<br>• Phân tích cơ chế AWS dành riêng 5 địa chỉ IP trong mỗi Subnet (`.0` Network, `.1` VPC Router, `.2` DNS, `.3` Future, `.255` Broadcast).<br>• Lập bảng quy hoạch IP tĩnh cho toàn bộ các dịch vụ của dự án RAG Capstone. | Hoàn thành sơ đồ ma trận mạng (Network CIDR Matrix) cho 6 Subnets trên 2 AZs. |
| **Thứ 3 (18/08)** | • Khởi tạo Custom VPC `Custom-RAG-VPC` với dải CIDR `10.0.0.0/16` qua AWS CLI.<br>• Kích hoạt tính năng `enableDnsHostnames` và `enableDnsSupport` cho VPC.<br>• Tạo 2 Public Subnets và 4 Private/Isolated Subnets tại `ap-southeast-1a` và `ap-southeast-1b`. | 6 Subnets được tạo thành công, gắn thẻ Tagging quy chuẩn theo tiêu chuẩn AWS Well-Architected. |
| **Thứ 4 (19/08)** | • Khởi tạo Internet Gateway `Custom-RAG-IGW` và liên kết (attach) trực tiếp vào Custom VPC.<br>• Tạo Public Route Table `Public-RTB`, tạo route `0.0.0.0/0` trỏ tới Internet Gateway.<br>• Gắn kết (associate) 2 Public Subnet vào `Public-RTB`. | Các tài nguyên trong Public Subnet sẵn sàng khả năng trao đổi hai chiều với mạng Internet. |
| **Thứ 5 (20/08)** | • Khởi tạo máy ảo EC2 kiểm thử trong Public Subnet và một máy ảo trong Private Subnet.<br>• Kiểm tra bảng định tuyến nội bộ: Kiểm tra khả năng gửi gói tin giữa hai máy ảo khác Subnet qua IP nội bộ `10.0.x.x`.<br>• Xác minh máy ảo Private không thể bị truy cập trực tiếp từ Internet. | Đường truyền mạng nội bộ giữa các AZs ổn định với độ trễ cực thấp (<1ms). |
| **Thứ 6 (21/08)** | • Cấu hình mô hình chuỗi tường lửa an toàn (**Security Group Chaining**):<br>  1. `ALB-SG`: Chỉ mở Inbound cổng 80/443 từ `0.0.0.0/0`.<br>  2. `EC2-App-SG`: Chỉ cho phép Inbound cổng 8000 (Backend) và 3000 (Frontend) xuất phát từ `ALB-SG`.<br>  3. `Database-SG`: Chỉ cho phép Inbound cổng 5432 (PostgreSQL) và 6333 (Qdrant) xuất phát từ `EC2-App-SG`. | Thiết lập thành công ranh giới Zero-Trust: Database và Compute được bảo vệ hoàn toàn khỏi Internet. |
| **Thứ 7 - CN (22-23/08)** | • Đo lường thông lượng và kiểm tra bảng định tuyến bằng các công cụ mạng `traceroute`, `nc` (Netcat).<br>• Tổng hợp sơ đồ kiến trúc mạng VPC và viết báo cáo kỹ thuật Tuần 3. | Bàn giao hạ tầng mạng VPC hoàn chỉnh theo đúng kế hoạch TTTN-02. |
