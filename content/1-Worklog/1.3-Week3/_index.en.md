...
| :--- | :--- | :--- |
| **Mon (17/08)** | • Studied CIDR block math and subnet masking (/16, /24, /28).<br>• Accounted for the 5 reserved IP addresses per subnet (`.0` Network, `.1` VPC Router, `.2` DNS, `.3` Future, `.255` Broadcast).<br>• Formulated a static IP allocation matrix for all RAG microservices. | Finalized Network CIDR Matrix across 6 Subnets spanning 2 Availability Zones. |
| **Tue (18/08)** | • Launched custom VPC `Custom-RAG-VPC` (`10.0.0.0/16`) via AWS CLI.<br>• Enabled `enableDnsHostnames` and `enableDnsSupport` VPC attributes.<br>• Provisioned 2 Public Subnets and 4 Private/Isolated Subnets across `ap-southeast-1a` and `ap-southeast-1b`. | 6 subnets created and verified with Well-Architected standard tags. |
| **Wed (19/08)** | • Provisioned Internet Gateway `Custom-RAG-IGW` and attached it to `Custom-RAG-VPC`.<br>• Created Public Route Table `Public-RTB` and configured default route `0.0.0.0/0` targeting the IGW.<br>• Bound both Public Subnets to `Public-RTB`. | Verified bidirectional Internet transit capability for public ingress components. |
| **Thu (20/08)** | • Deployed test EC2 instances inside Public Subnet and Private Subnet.<br>• Validated internal routing by testing cross-subnet packet transmission via private IPs (`10.0.x.x`).<br>• Verified that Private Subnet instances cannot be resolved or probed directly from external WAN. | Cross-AZ internal VPC throughput verified with ultra-low latency (<1ms). |
| **Fri (21/08)** | • Configured **Security Group Chaining** (Zero-Trust Firewall Matrix):<br>  1. `ALB-SG`: Inbound ports 80/443 open to `0.0.0.0/0`.<br>  2. `EC2-App-SG`: Inbound ports 8000 (Backend) and 3000 (Frontend) restricted exclusively to `ALB-SG`.<br>  3. `Database-SG`: Inbound ports 5432 (PostgreSQL) and 6333 (Qdrant) restricted exclusively to `EC2-App-SG`. | Enforced strict Zero-Trust boundaries: Database and compute tiers completely shielded from WAN. |
| **Sat - Sun (22-23/08)** | • Conducted routing and throughput audits using `traceroute` and `nc` (Netcat).<br>• Generated VPC architecture topology diagram and finalized Week 3 technical report. | Delivered complete enterprise VPC foundation per TTTN-02 milestones. |
