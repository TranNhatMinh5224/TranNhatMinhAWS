...
| :--- | :--- | :---: | :---: | :--- | :--- |
| **Mon** | • Conducted Serverless Cost Modeling: Compared 24/7 standing EC2 idle costs against on-demand AWS Lambda per-millisecond billing.<br>• For bursty enterprise legal ingestion, Serverless architecture delivers >85% baseline cost reduction.<br>• Analyzed *AWS Lambda Concurrency Limits and Execution Quotas*. | 24/08/2026 | 24/08/2026 | • [AWS Lambda Cost Optimization](https://aws.amazon.com/lambda/pricing/)<br>• [Understanding Lambda Execution Environment](https://docs.aws.amazon.com/lambda/latest/dg/runtimes-context.html) | Delivered comparative TCO report evaluating EC2 vs AWS Lambda cost envelopes. |
| **Tue** | • Developed Python Lambda handler `DocumentValidator`: Validated file MIME types and computed SHA-256 deduplication hashes.<br>• Packaged source code and dependencies into deployment ZIP bundles.<br>• Authored IAM Execution Role granting S3 read access and CloudWatch log emission. | 25/08/2026 | 25/08/2026 | • [Building Lambda Functions with Python](https://docs.aws.amazon.com/lambda/latest/dg/lambda-python.html)<br>• [Lambda Execution Role & Permissions](https://docs.aws.amazon.com/lambda/latest/dg/lambda-intro-execution-role.html) | Successfully deployed first Lambda function via AWS CLI; verified unit test executions. |
| **Wed** | • Configured S3 Event Trigger: Applied resource-based policy granting S3 permission to call `lambda:InvokeFunction`.<br>• Defined `s3:ObjectCreated:*` event filter scoped strictly to `incoming/` prefix.<br>• Uploaded test document to S3 and monitored automated execution logs in CloudWatch Log Streams. | 26/08/2026 | 26/08/2026 | • [Using AWS Lambda with Amazon S3](https://docs.aws.amazon.com/lambda/latest/dg/with-s3.html)<br>• [Amazon S3 Event Notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/EventNotifications.html) | Automated ingestion verified: S3 file upload triggers Lambda processing in under 120ms. |
| **Thu** | • Created Amazon API Gateway (HTTP API) serving as an external webhook ingestion endpoint.<br>• Configured Lambda Proxy Integration and CORS policies enabling browser client requests.<br>• Executed API invocation tests via Postman, measuring endpoint latency. | 27/08/2026 | 27/08/2026 | • [Amazon API Gateway HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api.html)<br>• [Enabling CORS for HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-cors.html) | Verified synchronous API response returning `200 OK` with 45ms average round-trip time. |
| **Fri** | • Cold Start Optimization: Benchmarked memory resizing from 128MB to 512MB (scaling proportional virtual CPU capacity on Nitro).<br>• Measured Init Phase reduction: Slashed cold boot latency from 1,850ms down to 380ms.<br>• Evaluated AWS Lambda 15-minute (900s) hard timeout constraints. | 28/08/2026 | 28/08/2026 | • [Operating Lambda: Performance Optimization](https://aws.amazon.com/blogs/compute/operating-lambda-performance-optimization-part-1/)<br>• [Serverless Land Best Practices](https://serverlessland.com/) | Established architectural rule: Lambda handles fast validation; heavy OCR tasks offload to Celery. |
| **Sat - Sun** | • Audited Lambda Execution Role, stripping all unneeded wildcards per Least Privilege.<br>• Packaged deployment artifacts and compiled Week 4 technical documentation. | 29/08/2026 | 30/08/2026 | • [Security Best Practices for AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/best-practices.html#security-best-practices)<br>• [Cloud Journey Serverless Module](https://cloudjourney.awsstudygroup.com/) | 100% of Serverless milestones completed per TTTN-02 schedule. |
