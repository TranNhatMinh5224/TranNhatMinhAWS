...
| :--- | :--- | :---: | :---: | :--- | :--- |
| **Mon** | • Enabled AWS CloudTrail **Multi-Region Trail** `Enterprise-Audit-Trail`.<br>• Directed all management events (Read/Write) to a dedicated encrypted S3 audit bucket secured with SSE-KMS.<br>• Enforced cryptographic Log File Integrity Validation using SHA-256 hash chains. | 31/08/2026 | 31/08/2026 | • [AWS CloudTrail User Guide](https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-user-guide.html)<br>• [CloudTrail Log File Integrity Validation](https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-log-file-validation-intro.html) | Global infrastructure actions logged immutably, ensuring tamper-evident operational compliance. |
| **Tue** | • Installed the **Amazon CloudWatch Unified Agent** package across the EC2 compute node.<br>• Authored `amazon-cloudwatch-agent.json` configuring 60s metric collection intervals for RAM, disk, and swap.<br>• Attached `CloudWatchAgentServerPolicy` to the EC2 IAM Instance Profile. | 01/09/2026 | 01/09/2026 | • [Installing CloudWatch Unified Agent](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/install-CloudWatch-Agent-on-EC2-Instance.html)<br>• [CloudWatch Custom Metrics Configuration](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/publishingMetrics.html) | Custom namespace `CWAgent` populated in CloudWatch Console with live memory utilization telemetry. |
| **Wed** | • Advanced Self-Directed Study (National Day holiday):<br>• Mastered **CloudWatch Logs Insights** query syntax: Filtered HTTP 5xx error events and computed request frequencies per source IP.<br>• Investigated CloudWatch Composite Alarms architecture. | 02/09/2026 | 02/09/2026 | • [CloudWatch Logs Insights Syntax](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/CWL_QuerySyntax.html)<br>• [Creating CloudWatch Composite Alarms](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Create_Composite_Alarm.html) | Developed rapid log forensic querying skills eliminating the need to download large raw log files. |
| **Thu** | • Deployed CloudWatch Metric Alarm `EC2-CPU-High-Utilization` monitoring `CPUUtilization` (>80% over two consecutive 5-minute evaluation periods).<br>• Deployed secondary alarm `EC2-Memory-High-Utilization` monitoring `mem_used_percent` (>85%).<br>• Bound both alarms to Amazon SNS Topic `System-Critical-Alerts`. | 03/09/2026 | 03/09/2026 | • [Amazon CloudWatch Alarms Guide](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html)<br>• [Amazon SNS Developer Guide](https://docs.aws.amazon.com/sns/latest/dg/welcome.html) | Automated alerting framework primed to protect the Capstone production node. |
| **Fri** | • Executed Synthetic Load Testing: Installed `stress-ng` on the EC2 host and triggered 95% CPU load for 15 minutes.<br>• Monitored CloudWatch telemetry: Observed status transition from `OK` to `ALARM` after 10 minutes.<br>• Verified email delivery: Received real-time SNS incident notification containing exact breach metrics. | 04/09/2026 | 04/09/2026 | • [EC2 Compute Stress Testing Methodologies](https://aws.amazon.com/blogs/compute/stress-testing-ec2-instances/)<br>• [CloudWatch Alarm State Transition Logic](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/alarm-evaluate-data-points.html) | End-to-end incident detection and notification workflow verified with 100% precision. |
| **Sat - Sun** | • Built executive **CloudWatch Operational Dashboard** integrating 4 primary telemetry widgets: CPU, Memory, Disk Space, and Network I/O.<br>• Documented configurations and compiled Week 5 technical worklog. | 05/09/2026 | 06/09/2026 | • [Amazon CloudWatch Dashboards](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Dashboards.html)<br>• [Cloud Journey Monitoring Module](https://cloudjourney.awsstudygroup.com/) | Delivered complete infrastructure observability framework meeting TTTN-02 criteria. |
