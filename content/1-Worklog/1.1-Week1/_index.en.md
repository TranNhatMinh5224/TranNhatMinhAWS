...
| :--- | :--- | :--- |
| **Mon (03/08)** | • Initialized AWS account under First Cloud AI Journey (FCAJ).<br>• Enabled Virtual MFA for Root User via authenticator application.<br>• Audited IAM credentials report confirming zero standing Root Access Keys. | Account verified; Level 1 compliance achieved per CIS AWS Foundations Benchmark. |
| **Tue (04/08)** | • Installed and configured AWS CLI v2 on Windows 11 workstation.<br>• Provisioned administrative IAM User `dev-admin` managed under `CloudDevelopers` group.<br>• Initialized local credential profiles in `~/.aws/credentials` and `~/.aws/config`. | Executed `aws sts get-caller-identity` returning verified IAM User identity under `ap-southeast-1`. |
| **Wed (05/08)** | • Activated *Receive CloudWatch Billing Alerts* in Billing Preferences.<br>• Created an AWS Budget capped at $10/month across the account.<br>• Built a CloudWatch Metric Alarm monitoring `EstimatedCharges` threshold at $5.00 USD in `us-east-1`. | Configured Amazon SNS Topic `Billing-Alerts-Topic`; confirmed email subscription endpoint. |
| **Thu (06/08)** | • Investigated AWS Global Infrastructure: Regions, Availability Zones (AZs), and Edge Locations.<br>• Conducted network latency (RTT) profiling from Vietnam to regional data centers (`ap-southeast-1`, `ap-east-1`).<br>• Studied *AWS Well-Architected Framework: Security & Cost Optimization Pillars*. | Selected Singapore (`ap-southeast-1`) as the primary region offering minimal latency (~35ms). |
| **Fri (07/08)** | • Attended Technical Orientation Briefing with AWS Senior Solutions Architects.<br>• Pitched Capstone proposal: *Enterprise Legal & Knowledge RAG Assistant on AWS*.<br>• Received feedback on multi-tier network isolation for sensitive enterprise data. | Capstone topic approved; finalized architectural baseline combining Multi-AZ VPC, Qdrant, and FastAPI. |
| **Sat - Sun (08-09/08)** | • Completed foundational assessments on the Cloud Journey training portal.<br>• Deep-dived into EC2 Nitro System virtualization concepts and EBS block storage architecture. | Scored 100% on foundational cloud computing entrance evaluation. |
